...
final class JsonEncoder {
    static {
        JsonEncoders.addEncoder(CharSequence.class, (val, builder) -> builder.append('"').append(val).append('"'));
        JsonEncoders.addEncoder(Map.class, JsonEncoder::encodeMap);
        JsonEncoders.addEncoder(Iterable.class, JsonEncoder::encodeIterable);
        JsonEncoders.addEncoder(Number.class, (val, builder) -> builder.append(val));
        JsonEncoders.addEncoder(Character.class, (val, builder) -> builder.append('"').append(val).append('"'));
        JsonEncoders.addEncoder(Boolean.class, (val, builder) -> builder.append(val));
        JsonEncoders.addEncoder(Object.class, (val, builder) -> {
            if (val.getClass().isArray()) {
                JsonEncoder.encodeArray(val, builder);
            } else {
                JsonEncoder.encodeObject(val, builder);
            }
        });
    }
    ...
    static void encodeArray(Object arr, StringBuilder builder) {
    ...
    static void encodeIterable(Iterable iter, StringBuilder builder) {
        builder.append('[');
    ...
    static void encodeMap(Map map, StringBuilder builder) {
        builder.append('{');
    ...
    static void encodeObject(Object obj, StringBuilder builder) {
        builder.append('{');
    ...
    static void appendValue(Object value, StringBuilder builder) {
        if (value == null) {
            builder.append("null");
            return;
        }
        ...
        if (cls.isPrimitive()) {
            ...
            final EncoderFunc dec = JsonEncoders.INST.get(cls);
            dec.encode(value, builder);
        }
    }
}
