...
public class JsonParsers2 {
    private final Map<Class<?>, Class<? extends JsonParser>> interfaceParsers = new HashMap<Class<?>, Class<? extends JsonParser>>() {
        @Override
        public Class<? extends JsonParser> get(Object key) {
            if (key instanceof Class<?>) {
                final Class<?> clsKey = (Class<?>)key;
                Class<? extends JsonParser> res = super.get(key);

                if (res == null) {
                    Class<?> cls;
                    for (Class<?> c : super.keySet()) {
                        cls = c;
                        do {
                            cls = cls.getSuperclass();

                            if (clsKey.equals(cls)) {
                                res = super.get(cls);
                                break;
                            }
    ...
    private final Map<Class<?>, Class<? extends JsonParser>> classParsers = new HashMap<>();

    public JsonParser get(Class<?> cls) throws ReflectiveOperationException {
        Class<? extends JsonParser> jsonParserCls;

        if (cls.isInterface()) {
            jsonParserCls = interfaceParsers.get(cls);
        } else {
            jsonParserCls = classParsers.get(cls);
        }

        return jsonParserCls.getConstructor().newInstance();
    }
